...
def start_client():
    ...
    try:
        while True:
            ...
            while True:
                try:
                    ...
                    print(f"{Colors.WARNING}Invalid input. Please enter a positive number for file size.{Colors.RESET}")

            while True:
                try:
                    tcp_connections = int(input(f"{Colors.BOLD}Enter number of TCP connections: {Colors.RESET}"))
                    if tcp_connections <= 0:
                        print(f"{Colors.WARNING}Number of TCP connections must be a positive integer.{Colors.RESET}")
                        continue
                    break
                except ValueError:
                    print(f"{Colors.WARNING}Invalid input. Please enter a positive integer.{Colors.RESET}")

            while True:
                try:
                    udp_connections = int(input(f"{Colors.BOLD}Enter number of UDP connections: {Colors.RESET}"))
                    if udp_connections <= 0:
                        print(f"{Colors.WARNING}Number of UDP connections must be a positive integer.{Colors.RESET}")
                        continue
                    break
                except ValueError:
                    print(f"{Colors.WARNING}Invalid input. Please enter a positive integer.{Colors.RESET}")
            ...
            for conn_id, duration, speed in tcp_stats:
                print(f"{Colors.SUCCESS}TCP transfer #{conn_id}: {duration:.2f} s, {speed:.2f} bps{Colors.RESET}")
        ...
        print(f"{Colors.ERROR}Client interrupted. Exiting...{Colors.RESET}")
    except Exception as e:
